...
## Where the concept references live — read this before cloning

⚠️ **The reference sheets are not on `main`.** They are on the branch
`agent/asset-artist/3d1472635e40` under `art/production/`, which exists on `origin` and nowhere
else. **That branch must not be deleted.** It is the only copy of the round-1 and round-2
candidates, every rejected pass, and the two manifests (`handoff-manifest.md`,
`handoff-manifest-round2.md`) that record what was corrected and why.

```
git fetch origin agent/asset-artist/3d1472635e40
git checkout FETCH_HEAD -- art/production
```

They were deliberately left off `main`: they are ~20 MB of JPEG concept renders, not production
assets, and `main` is a Godot project. If a human artist would rather have them in the tree, say
so and they can be landed under `art/production/` in their own commit — that is a preference, not
a blocker.

## How much to trust the concept references

The manifests' provenance is worth reading before starting — several corrections are only legible
as the difference between two passes.

The references are **Art-Director-reviewed but not Visual-Identity-Critic-validated.** The round-1
batch went through the full critic gate; round 2 did not. Trust them for material language, value
...
Two asset passes failed the black-fill test at 64 × 72 and the failure was reported as a probable
...
The restoration sequence in row 1 keys off §8's timeline, whose endpoints are **derived** from the
propagation speed and R6's authored size rather than authored as fixed durations. If R6 is ever
re-authored, those numbers move — read §4.2 before hand-keying the frames.
